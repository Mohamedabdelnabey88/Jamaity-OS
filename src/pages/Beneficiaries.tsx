...
export default function Beneficiaries() {
  ...
  return <main className="module-page">
    ...
    <section className="data-card">
      ...
      {loading ? <div className="empty">جاري تحميل البيانات...</div> : rows.length === 0 ? <div className="empty"><UserRound size={32}/><div>لا يوجد مستفيدون حتى الآن.</div></div> : <div className="table-wrap"><table><thead><tr><th>المستفيد</th><th>الجوال</th><th>الحالة</th><th>تاريخ التسجيل</th><th></th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td><div className="person"><div className="person-avatar">{row.full_name.slice(0,1)}</div><div><b>{row.full_name}</b><small>ملف مستفيد</small></div></div></td><td>{row.phone || '—'}</td><td><span className={`status ${row.status}`}>{row.status==='active'?'نشط':row.status==='blocked'?'موقوف':'غير نشط'}</span></td><td>{new Date(row.created_at).toLocaleDateString('ar-SA')}</td><td><button className="icon"><Eye size={17}/></button></td></tr>)}</tbody></table></div>}
    </section>
  </main>
}
