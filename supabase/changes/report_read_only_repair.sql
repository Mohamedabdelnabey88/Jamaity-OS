...
begin
 ...
 if can_governance then
  -- A report must remain read-only, including when called through PostgREST.
  select jsonb_build_object('score',case when count(*)=0 then null else round(100.0*count(*) filter(where status='ready')/count(*)) end,
   'ready',count(*) filter(where status='ready'),'total',count(*),
   'overdue',count(*) filter(where due_date<current_date and status not in ('ready','not_applicable')))
  into governance from public.governance_requirements where charity_id=c and status<>'not_applicable';
 end if;
