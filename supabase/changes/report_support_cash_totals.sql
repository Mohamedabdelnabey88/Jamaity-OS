-- Repair cash support totals without changing stored records or report access.
-- Both current cash and legacy financial records are monetary; in-kind valuations are excluded.
...
begin
 ...
 with source as (
  ...
  'cash_amount_total',(select coalesce(sum(amount) filter(where (p_kind='donations' and type='cash') or (p_kind='support' and type in ('cash','financial'))),0) from filtered),
